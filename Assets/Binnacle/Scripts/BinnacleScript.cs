...
public class BinnacleScript : MonoBehaviour {
	...
	[Tooltip("This will define how far away from the player a marker needs to be to be culled. This also works with sticky edge markers.")]
	public float markerCull = 1.5f;
	[Tooltip("This will define how far away from the player a marker needs to be to stick to the edge of the radar. If you wish to disable this, then set it the same as the 'markerCull'")]
	public float stickyMarkerPosition = 1f;
	...
	void DrawRadarMarkers(){
		...
		foreach(RadarObject radarObject in radarMarkers){
			...
			//Sticky Markers
			if (distanceToObject > stickyMarkerPosition){
				if(distanceToObject < markerCull){
					distanceToObject = stickyMarkerPosition;
					radarObject.icon.transform.localScale = new Vector3(1, 1, 1);
				} else if (distanceToObject > markerCull) {
					radarObject.icon.transform.localScale = new Vector3(0, 0, 0);
					continue;
				}
			}
